...
export type ExtractProps<T> = Omit<T, `render${string}`> & Partial<Pick<T, keyof T & `render${string}`>>
...
> = DefineSetupFnComponent<ExtractProps<T>, ExtractEvent<T>, SlotsType<ExtractSlots<T> & S>> & {
  new (...args: any[]): Expose
}
